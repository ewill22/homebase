...
- `guapa_pm.py` — daily 7:15 AM PM briefing across both Guapa repos (git log + catalog/coffee/editorial metrics, scored against a roadmap). Writes `pm-reports/pm-YYYY-MM-DD.md` + `pm-reports/task-queue.md` under `C:\Users\eewil\guapa\guapa-pm\` (operational output, not in this repo), and emails the HTML-styled briefing via `emailer.send_email`. Sends as its own separate email — does not piggyback on the morning summary. Schedule task XML lives at `guapa-pm-task.xml`; pythonw logs to `C:\Users\eewil\guapa\guapa-pm\guapa_pm.log`.
  ...
  Conventions worth knowing before editing the briefing:
  - **`auto_shipping: True`** on a `ROADMAP` entry means it's actually running daily (Spotify rotation, release-date backfill, etc.). The honest-status logic uses it to decide `[SHIPPING]` (forest green pill) vs `[TODO]` (yellow pill). Anything `active` without this flag is human-time work — surface that backlog instead of pretending every line item is in motion.
  - **Status pill palette** (`_PILL_STYLES`) drawn from a real photo, kept in one map so changing accents stays surgical: SHIPPING `#588157`, TODO `#f0c014`, BLOCKED `#b85c38`, PARKED `#888`, DONE `#3a5a40`. Section headers use the deep forest `#3a5a40` to match.
  - **Top-of-briefing nag banner** renders for any ROADMAP item with `team=eric` AND `status in (active, blocked)` AND a `since: "YYYY-MM-DD"` field. Day counter grows each morning until the item is set to `done`. Currently nagging the LLC and trademark filings.
  - **RECOMMENDED NEXT** is the AI-dev-team queue: filtered to `status=active` AND `team in (backend, frontend)` AND NOT `auto_shipping`. Eric-team items stay on the top nag, external blockers stay quiet, auto-shipping work shows progress in ROADMAP not here.
